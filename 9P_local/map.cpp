...
void Map<KEY,T>::destructCode(Elem *& p) {
    if (p->left != nullptr && p->left != _root)  //delete recursively
        destructCode(p->left);
...
bool Map<KEY, T>::erase(KEY key) {
    ...
    Iterator pred = begin();    //find the predecessor
        ...
        if (key < cur->key) {                                   //look to the left
            ...
            if (cur->left == nullptr && (cur->right == nullptr || cur->rightThread)) //no children
                ...
                *p = cur->right;
                ...
                skip = true;
                Elem* right_min = cur->right;     //for two child case, maintain threads
                ...
                if (right_min_parent == cur) {    //right min is immediate child
                    ...
                    right_min_parent->rightThread = right_min->rightThread;
                }
                else {
    ...
    if (!skip && predecessor != _root) {  //for cases that are not two child, fix threading
        if (predecessor->rightThread || predecessor->right == nullptr) {  //make sure right thread is correct
...
typename Map<KEY, T>::Iterator Map<KEY, T>::Iterator::operator++(int) {
    if (_cur != nullptr) {
        if (_cur != nullptr && _cur->rightThread)  //if right threaded, follow thread
            _cur = _cur->right;
        else {
            if (_cur->right == nullptr)
                _cur = nullptr;
            else {
                Elem *right_min = _cur->right;     //else find successor as right min
                while (right_min->left != nullptr)
                    right_min = right_min->left;
                _cur = right_min;
            }
